...
module serv_rf_ram_if
  ...
  (
   ...
   assign wtrig1 = (ratio == 2) ? wcnt[0] : wtrig0_r;
   ...
   assign o_waddr = (width == 32) ? wreg : {wreg, wcnt[CMSB:l2r]};
   ...
   // Simplified from generate if (width == 32)
   assign o_raddr = (width == 32) ? rreg : {rreg, rcnt[CMSB:l2r]};
   ...
   // Simplified from generate if (ratio == 2)
   assign o_ren = (ratio == 2) ? rgate : (rgate & (rcnt[l2r-1:1] == 0));
   ...
   always @(posedge clk) begin
      ...
      rgate <= (i_rst && (reset_strategy != "NONE")) ? 1'b0 : 
               (&rcnt | i_rreq) ? i_rreq : rgate;

      rtrig1 <= rtrig0;
      
      rcnt <= (i_rst && (reset_strategy != "NONE")) ? {CMSB+1{1'b0}} :
              (i_rreq | i_wreq) ? {{CMSB-1{1'b0}},i_wreq,1'b0} : rcnt+{{CMSB{1'b0}},1'b1};

      rreq_r <= (i_rst && (reset_strategy != "NONE")) ? 1'b0 : i_rreq;
      
      rgnt <= (i_rst && (reset_strategy != "NONE")) ? 1'b0 : rreq_r;
