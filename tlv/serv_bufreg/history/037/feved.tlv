...
\TLV
   |default
      @0
         /gen_lsb_w4[0:0]  // Replicated scope for gen_lsb_w_4 generate for loop
            // TLV assignments for replicated scope (when W == 4)
            <<1$lsb[1:0] = (*i_en && *i_cnt0) ? |default$qq[1 : 0] : $lsb;
            <<1$data_tail[W-2:0] = *i_en ? (*data[B : 1] & {B{~ *i_cnt_done}}) : $data_tail;
         // clr_lsb_internal logic migrated from generate blocks
         $clr_lsb_internal[B:0] = (B > 0) ? {{B{1'b0}}, (*i_cnt0 & *i_clr_lsb)} : (*i_cnt0 & *i_clr_lsb);
         ...
         $shift_amount[2:0] = ! *i_shift_op ? 3'd3 : 
                              *i_right_shift_op ? (3'd3 + {1'b0, *i_shamt[1 : 0]}) :
                              ({1'b0, ~ *i_shamt[1 : 0]});
         $muxdata[2 * W + B - 2:0] = {*data[W + B - 1 : 0], /gen_lsb_w4[0]$data_tail};
         $muxout[B:0] = $muxdata[({1'b0, $shift_amount}) +: W];
         $lsb_w_eq_4[1:0] = /gen_lsb_w4[0]$lsb;
         $qq_w_eq_4[B:0] = *i_en ? $muxout : {W{1'b0}};
         
         // Output assignments (migrated from generate if blocks)
         *o_lsb = (*MDU & *i_mdu_op) ? 2'b00 : 
                  (W == 1) ? $lsb_w_eq_1 : $lsb_w_eq_4;
         *o_q = (W == 1) ? $qq_w_eq_1 : $qq_w_eq_4;
         ...
         *o_ext_rs1 = *data;
         ...
         \SV_plus
            ...
            always \@(posedge clock) begin
               //Make sure carry is cleared before loading new data (unified expression)
               $$cc_r[B:0] <= {{B{1'b0}}, $cc & *i_en};
            end
            ...
            generate
               ...
               for (gen_lsb_w_4_i = (W != 4); gen_lsb_w_4_i < 1; gen_lsb_w_4_i++) begin : gen_lsb_w_4
                  ...
                  always \@(posedge clock) begin
                     // lsb and data_tail assignments moved to TLV replicated scope above
