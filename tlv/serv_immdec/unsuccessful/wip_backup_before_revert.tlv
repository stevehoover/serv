...
\TLV
   |default
      @0
         /gen_ww1[W == 1 ? 0 \: -1 : 0]
            /gen_shared[! SHARED_RFADDR_IMM_REGS ? 0 \: -1 : 0]
            /gen_separate[SHARED_RFADDR_IMM_REGS ? 0 \: -1 : 0]
         /gen_ww4[W != 1 ? 0 \: -1 : 0]
         \SV_plus
            ...
            generate
               // if (W == 1)
               genvar gen_w1;
               for (gen_w1 = (W != 1); gen_w1 < 1; gen_w1++) begin : ww1
               reg               imm31;
               ...
               wire       signbit = imm31 & ! i_csr_imm_en;
                  ...
                  genvar gen_shared;
                  for (gen_shared = ! SHARED_RFADDR_IMM_REGS; gen_shared < 1; gen_shared++) begin : shared
                  ...
                  // if (!SHARED_RFADDR_IMM_REGS)
                  genvar gen_separate;
                  for (gen_separate = SHARED_RFADDR_IMM_REGS; gen_separate < 1; gen_separate++) begin : separate
                     reg [4:0]  rd_addr;
                     reg [4:0]  rs1_addr;
                     reg [4:0]  rs2_addr;
                  ...
                  for (gen_shared_addr = ! SHARED_RFADDR_IMM_REGS; gen_shared_addr < 1; gen_shared_addr++) begin : shared_addr
                  ...
                  for (gen_separate_addr = SHARED_RFADDR_IMM_REGS; gen_separate_addr < 1; gen_separate_addr++) begin : separate_addr
                     assign o_rd_addr  = separate[0].rd_addr;
                     assign o_rs1_addr = separate[0].rs1_addr;
                     assign o_rs2_addr = separate[0].rs2_addr;
                  end
               ...
               // if (W != 1)
               genvar gen_w4;
               for (gen_w4 = (W == 1); gen_w4 < 1; gen_w4++) begin : ww4
               reg [4:0]         rd_addr;
               reg [4:0]         rs1_addr;
               reg [4:0]         rs2_addr;
               ...
               always \@(posedge i_clk) begin
                  ...
                  {i31, rs2_addr, rs1_addr, rd_addr} <= i_wb_en ? {i_wb_rdt[31], i_wb_rdt[24:20], i_wb_rdt[19:15], i_wb_rdt[11:7]} : 
                                                                   {i31, rs2_addr, rs1_addr, rd_addr};
                  ...
                  {i23, i10} <= i_cnt_en ? {i27, i27} :
                                i_wb_en  ? {i_wb_rdt[23], i_wb_rdt[10]} :
                                           {i23, i10};
                  
                  i27 <= i_cnt_en ? (i_ctrl[2] ? i7 : i_ctrl[1] ? signbit : i20) :
                         i_wb_en  ? i_wb_rdt[27] :
                                    i27;
                  
                  {i20, i15} <= i_cnt_en ? {i15, i19} :
                                i_wb_en  ? {i_wb_rdt[20], i_wb_rdt[15]} :
                                           {i20, i15};
                  
                  {i19, i18, i17} <= i_cnt_en ? (i_ctrl[3] ? {signbit, signbit, signbit} : {i23, i22, i21}) :
                                     i_wb_en  ? {i_wb_rdt[19], i_wb_rdt[18], i_wb_rdt[17]} :
                                                {i19, i18, i17};
            
                  //Bit lane 2
                  {i22, i9} <= i_cnt_en ? {i26, i26} :
                               i_wb_en  ? {i_wb_rdt[22], i_wb_rdt[9]} :
                                          {i22, i9};
                  
                  {i26, i14} <= i_cnt_en ? {i30, i18} :
                                i_wb_en  ? {i_wb_rdt[26], i_wb_rdt[14]} :
                                           {i26, i14};
                  
                  {i30, i29, i28} <= i_cnt_en ? ((i_ctrl[1] | i_ctrl[2]) ? {signbit, signbit, signbit} : {i14, i13, i12}) :
                                     i_wb_en  ? {i_wb_rdt[30], i_wb_rdt[29], i_wb_rdt[28]} :
                                                {i30, i29, i28};
            
                  //Bit lane 1
                  {i21, i8} <= i_cnt_en ? {i25, i25} :
                               i_wb_en  ? {i_wb_rdt[21], i_wb_rdt[8]} :
                                          {i21, i8};
                  
                  {i25, i13} <= i_cnt_en ? {i29, i17} :
                                i_wb_en  ? {i_wb_rdt[25], i_wb_rdt[13]} :
                                           {i25, i13};
                  
                  //Bit lane 0
                  {ii20_b2, ii7_b2} <= i_cnt_en ? {i24, i11} :
                                        i_wb_en  ? {i_wb_rdt[20], i_wb_rdt[7]} :
                                                   {ii20_b2, ii7_b2};
                  
                  {i24, i11} <= i_cnt_en ? {i28, i28} :
                  ...
                  i16 <= i_cnt_en ? (i_ctrl[3] ? signbit : ii20_b2) :
                         i_wb_en  ? i_wb_rdt[16] :
                                    i16;
            ...
            generate
               // if (W == 1)
               genvar gen_w1_csr;
               for (gen_w1_csr = (W != 1); gen_w1_csr < 1; gen_w1_csr++) begin : ww1_csr
                  assign o_csr_imm = ww1[0].imm_b19_b12_b20[4];
               end
               // if (W != 1)
               genvar gen_w4_csr;
               for (gen_w4_csr = (W == 1); gen_w4_csr < 1; gen_w4_csr++) begin : ww4_csr
                  assign o_csr_imm = {ww4[0].i18, ww4[0].i17, ww4[0].i16, ww4[0].i15};
               end
            endgenerate
            ...
            generate
               // if (W == 1)
               genvar gen_w1_imm;
               for (gen_w1_imm = (W != 1); gen_w1_imm < 1; gen_w1_imm++) begin : ww1_imm
                  assign o_imm = i_cnt_done ? ww1[0].signbit : i_ctrl[0] ? ww1[0].imm_b11_b7[0] : ww1[0].imm_b24_b20[0];
               end
               // if (W != 1)
               genvar gen_w4_imm;
               for (gen_w4_imm = (W == 1); gen_w4_imm < 1; gen_w4_imm++) begin : ww4_imm
                  assign o_imm = {(i_cnt_done ? ww4[0].signbit : (i_ctrl[0] ? ww4[0].i10 : ww4[0].i23)),
                                  i_ctrl[0] ? ww4[0].i9 : ww4[0].i22,
                      i_ctrl[0] ? ww4[0].i8 : ww4[0].i21,
                      i_ctrl[0] ? ww4[0].ii7_b2 : ww4[0].ii20_b2};
   end
            endgenerate
\SV
endmodule
