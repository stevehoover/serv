...
// The guts of serv_decode module.
\TLV serv_decode(/_top)
   |default
      @0
         // Core decode logic
         ...
         // funct3-based decode signals
         $co_sh_right = $funct3[2];
         $co_bne_or_bge = $funct3[0];

         // System operation decode
         ...
         // opcode & funct3 & op21 combinations
         ...
         // [2]: opcode[4] & !opcode[0]
         // [3]: opcode[4]
         $co_immdec_ctrl[3:0] = {$opcode[4], 
                                 $opcode[4] & !$opcode[0], 
                                 ($opcode[1:0] == 2'b00) | ($opcode[2:1] == 2'b00), 
                                 $opcode[3:0] == 4'b1000};
         ...
         // [1]: J U 
         // [0]: B S
         $co_immdec_en[3:0] = {$opcode[4] | $opcode[3] | $opcode[2] | !$opcode[0],                 //B I J S U
                               ($opcode[4] & $opcode[2]) | !$opcode[3] | $opcode[0],               //  I J   U
                               ($opcode[2:1] == 2'b01) | ($opcode[2] & $opcode[0]) | $co_csr_imm_en,//    J   U
                               ~$co_rd_op};                                                       //B     S
         ...
         // [1]: SLT*
         // [0]: Add/sub
         $co_alu_rd_sel[2:0] = {$funct3[2],                        //Bool
                                ($funct3[2:1] == 2'b01),         //SLT*
            ...
            generate
               if (PRE_REGISTER) begin : gen_pre_register
                  ...
                  always @(posedge clk) begin
                     $$funct3[2:0] <= $wb_en ? $wb_rdt[14 : 12] : $funct3;
                     $$imm30  <= $wb_en ? $wb_rdt[30] : $imm30;
                     $$imm25  <= $wb_en ? $wb_rdt[25] : $imm25;
                     $$opcode[4:0] <= $wb_en ? $wb_rdt[6 : 2] : $opcode;
                     $$op20   <= $wb_en ? $wb_rdt[20] : $op20;
                     $$op21   <= $wb_en ? $wb_rdt[21] : $op21;
                     $$op22   <= $wb_en ? $wb_rdt[22] : $op22;
                     $$op26   <= $wb_en ? $wb_rdt[26] : $op26;
                  end
...
\TLV
   ...
   // Use the TLV macro for the decoder logic
   m5+serv_decode(/_top)
   |default
      @0
